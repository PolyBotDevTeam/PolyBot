...
games_rules = '''Правила игр.
            ...
            3.2. Игроки всегда должны быть готовы предоставить скриншоты своих ходов по запросу противника, начиная с хода реквеста. Так, например, если на пятом ходу игрок А попросил игрока Б делать скриншоты, то к концу партии игрок Б обязан предоставить скриншоты всех ходов после пятого. Также можно запросить лишь один скриншот (на текущий момент). Делать скриншоты можно в начале, в конце или во время хода. Важно отметить, что скриншоты предоставляются по окончании партии, а не во время. Если игрок не может выполнить данное требование (скриншотов нет или отсутствует значительная их часть), игрок получает предупреждение, а игра заканчивается его поражением. Несмотря на то, что каждый игрок имеет право на запрос скриншотов, не стоит злоупотреблять этим правом без весомой причины.'''
